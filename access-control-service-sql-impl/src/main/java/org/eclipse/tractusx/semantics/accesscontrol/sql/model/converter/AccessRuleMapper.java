...
import org.eclipse.tractusx.semantics.accesscontrol.sql.rest.model.ListOperatorType;
import org.eclipse.tractusx.semantics.accesscontrol.sql.rest.model.ReadUpdateAccessRule;
import org.eclipse.tractusx.semantics.accesscontrol.sql.rest.model.SingleOperatorType;
...
public interface AccessRuleMapper {
   ...
   PolicyOperator map( SingleOperatorType source );

   @ValueMappings( {
         @ValueMapping( target = "INCLUDES", source = "INCLUDES" )
   } )
   PolicyOperator map( ListOperatorType source );

   @ValueMappings( {
         @ValueMapping( target = "EQ", source = "EQUALS" ),
         @ValueMapping( target = MappingConstants.NULL, source = "INCLUDES" )
   } )
   SingleOperatorType mapSingleOperator( PolicyOperator source );

   @ValueMappings( {
         @ValueMapping( target = "INCLUDES", source = "INCLUDES" ),
         @ValueMapping( target = MappingConstants.NULL, source = "EQUALS" )
   } )
   ListOperatorType mapListOperator( PolicyOperator source );
